...
def r2_key_for(kind: str, series: str, slug: str, mode: str | None, filename_slug: str, target_ext: str) -> str:
    if kind == "audio":
        return f"audio/{series}/{slug}/{mode or 'tts'}{target_ext}"
    if kind == "video":
...
def resolve_publish_collisions(items: list[dict]) -> None:
    ...
    for item in items:
        ...
        if item["kind"] != "audio":
            continue
        slug_mode = (item["slug"], item["mode"] or "tts")
        ...
        if previous_audio and previous_audio["sha1"] != item["sha1"]:
            ...
            item["id"] = slugify(item["r2Key"])
            item.setdefault("publishNotes", []).append(f"audio slug/mode disambiguated from {old_slug}/{slug_mode[1]}")
            slug_mode = (item["slug"], item["mode"] or "tts")
...
def emit_d1_sql(manifest: dict, output: Path) -> None:
    rows = [item for item in manifest["items"] if item["kind"] == "audio" and item.get("publicUrl")]
    lines = [
        "-- Generated by scripts/build_media_manifest.py",
